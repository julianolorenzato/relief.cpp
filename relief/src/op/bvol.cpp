...
void BoundingVolumeOp::buildBoxMesh(mesh::Mesh& mesh, const Eigen::Vector3d& center,
    ...
    for (int i = 0; i < 8; i++) {
        mesh.vertices[i].pos = center + signs[i][0] * halfExtents.x() * axes[0] +
    ...
    for (int f = 0; f < 12; f++)
        for (int c = 0; c < 3; c++) mesh.faces[f].w[c] = tris[f][c];
...
void BoundingVolumeOp::applyOBB(mesh::Mesh& mesh) const {
    ...
    Eigen::Vector3d worldCenter =
        mean + localCenter.x() * axes[0] + localCenter.y() * axes[1] + localCenter.z() * axes[2];
